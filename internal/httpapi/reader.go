...
// load remote resources or be framed by another site.
const readCSP = "default-src 'none'; img-src 'self'; style-src 'self'; font-src 'self'; " +
...
func (s *Server) getTOC(w http.ResponseWriter, r *http.Request) {
	...
	defer done()
	writeJSON(w, http.StatusOK, map[string]any{"chapters": len(e.Spine), "toc": e.TOC()})
}

// resourceType is the allowlist for /read/{id}/res: images (not SVG), CSS and
// fonts. It returns the media type to serve, or "".
func resourceType(declared string) string {
...
func (s *Server) bookURLs(id int64, e *formats.EPUB, dir string) sanitize.Options {
	return sanitize.Options{
		Resource: func(ref string) (string, bool) {
			...
			idx, ok := e.IndexOf(p)
			if !ok || resourceType(e.Manifest[idx].MediaType) == "" {
				return "", false
			}
...
func (s *Server) getChapter(w http.ResponseWriter, r *http.Request) {
	...
	body, sheets := sanitize.Chapter(src, s.bookURLs(it.ID, e, path.Dir(e.Manifest[idx].Href)))
	h := w.Header()
	h.Set("Content-Type", "text/html; charset=utf-8")
	h.Set("Content-Security-Policy", readCSP)
	h.Set("Cache-Control", "private, max-age=3600")
	w.Write(sanitize.Document(body, sheets, "/read/base.css"))
}
...
func (s *Server) getResource(w http.ResponseWriter, r *http.Request) {
	...
	typ := resourceType(e.Manifest[idx].MediaType)
	if typ == "" {
		s.fail(w, r, errNotFound)
		return
	}
